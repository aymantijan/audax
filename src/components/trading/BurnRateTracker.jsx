...
export default function BurnRateTracker({ trades, accountValue, currency = 'USD' }) {
  ...
  return (
    <Card title="Burn Rate (7-day window)">
      {noLosses ? (
        <div className="text-sm text-good">No losses in the last 7 days. Burn rate: {fmtMoney(0, 0, currency)}.</div>
      ) : (
        <div className="space-y-3">
          <div>
            <div className="text-3xl font-bold" style={{ color: LEVEL_COLOR[br.level] }}>
              {fmtMoney(br.avgDailyLoss, 0, currency)}
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            <div>
              ...
              <div className="font-semibold">{fmtMoney(br.monthlyLossProjection, 0, currency)}</div>
            </div>
            <div>
