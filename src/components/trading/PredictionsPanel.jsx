...
export default function PredictionsPanel({ account, trades }) {
          ...
          <>
            <ResponsiveContainer width="100%" height={220}>
              <AreaChart data={bands.bands}>
                ...
                <Tooltip {...tooltipStyle} formatter={(v, name) => [fmtMoney(v), name === 'p90' ? 'Optimistic (p90)' : name === 'p10' ? 'Pessimistic (p10)' : 'Median']} />
            ...
            <>
              ...
              {daysToTarget && !daysToTarget.insufficientData && !daysToTarget.alreadyMet && daysToTarget.daysRemaining != null && (
                <p className="text-[11px] text-mute mt-3 flex items-center gap-1.5">
                  ...
                  At your current avg. pace ({fmtMoney(daysToTarget.avgPerDay)}/trading day), reaching the profit target linearly would take ~{daysToTarget.daysRemaining} more day(s).
                </p>
              )}
              <p className="text-[11px] text-mute mt-2">
